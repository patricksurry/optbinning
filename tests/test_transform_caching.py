"""
Test transform caching for OptimalBinning.
"""
...
class TestTransformCaching:
    """Test suite for transform caching functionality."""

    def test_cache_creation_on_first_transform(self):
        """Test that cache is created on first transform."""
        np.random.seed(42)
        ...
        result1 = optb.transform(x, metric='woe')
        assert len(optb._transform_cache) == 1
        assert result1.shape == (1000,)

    def test_cache_reuse_on_second_transform(self):
        """Test that cache is reused on subsequent transforms with same parameters."""
        np.random.seed(42)
        x = np.random.randn(1000)
        y = (x + np.random.randn(1000) * 0.5 > 0).astype(int)
        ...
        result2 = optb.transform(x, metric='woe')
    ...
    def test_multiple_cache_entries_for_different_metrics(self):
        """Test that different metrics create separate cache entries."""
        np.random.seed(42)
        x = np.random.randn(1000)
        y = (x + np.random.randn(1000) * 0.5 > 0).astype(int)
        ...
        result_woe = optb.transform(x, metric='woe')
        assert len(optb._transform_cache) == 1

        result_event_rate = optb.transform(x, metric='event_rate')
        ...
        assert not np.allclose(result_woe, result_event_rate)
        assert not np.allclose(result_woe, result_indices)

    def test_cache_key_includes_all_parameters(self):
        """Test that cache key includes metric, metric_special, metric_missing, and show_digits."""
        np.random.seed(42)
        x = np.random.randn(1000)
        y = (x + np.random.randn(1000) * 0.5 > 0).astype(int)
        ...
        result1 = optb.transform(x, metric='woe', metric_special=0)
        result2 = optb.transform(x, metric='woe', metric_special=1)
        assert len(optb._transform_cache) == 2

        # Same metric but different metric_missing
        result3 = optb.transform(x, metric='woe', metric_missing=1)
        assert len(optb._transform_cache) == 3
    ...
    def test_cached_results_match_original_implementation(self):
        """Test that cached results match what the original implementation would produce."""
        np.random.seed(42)
        x = np.random.randn(500)
        y = (x + np.random.randn(500) * 0.5 > 0).astype(int)
        ...
        result1 = optb.transform(x_test, metric='woe')

        # Second transform (uses cache)
        result2 = optb.transform(x_test, metric='woe')

        # Results should be exactly identical (not just close)
        assert np.array_equal(result1, result2)

    def test_cache_with_different_input_arrays(self):
        """Test that cache works correctly with different input arrays."""
        np.random.seed(42)
        x = np.random.randn(1000)
        y = (x + np.random.randn(1000) * 0.5 > 0).astype(int)
        ...
        result1 = optb.transform(x_test1, metric='woe')
        result2 = optb.transform(x_test2, metric='woe')
    ...
    def test_cache_performance_benefit(self):
        ...
        y_train = (x_train + np.random.randn(1000) * 0.5 > 0).astype(int)
        ...
        for x_test in x_test_samples:
            results_cached.append(optb.transform(x_test, metric='woe'))
        ...
        for x_test in x_test_samples:
            ...
            results_uncached.append(optb.transform(x_test, metric='woe'))
        ...
        # Caching should provide measurable speedup (at least 1.1x)
